...
$movie_id = $_GET['id'];
$userid=$_SESSION['userid'];
$sql=mysql_query("SELECT * FROM rating2 where MovieId='$movie_id' and UserId='$userid'");
...
$rating = mysql_query("SELECT rating  FROM `rating2` WHERE `MovieId`='$movie_id'");
...
$user=$_SESSION["userid"];
		...
		if (isset($_SESSION["userid"]) && ($_SESSION["username"])&& $user_count == 0 ){
			...
			echo "
		<form action='ratebangla.php' method='POST'>
			<br/>Rate :  <select name='rating'>
				<option>1</option>
				<option>2</option>
				<option>3</option>
				<option>4</option>
	...
	";
			...
			echo "<font color='#945'>Rating : ".round($finalrate,2)."</font><br/><br/>";
			...
			echo "<font color='#945'>Rating : Please log in to rate or see the rating!</font>";
		}
